...
export function registerNodeCommands(program: Command) {
    ...
    .action(async () => {
      pm2.describe('validator', async (err, descriptions) => {
        ...
        if (fs.existsSync(path.join(__dirname, `../${File.SECRETS}`))) {
          ...
          console.error('Please start the node once before using status command')
          pm2.disconnect()
          return
        }
        ...
        if (status.status !== 'stopped') {
          ...
          const lockedStakeStr = accountInfo.lockedStake ? ethers.utils.formatEther(accountInfo.lockedStake) : ''
        ...
        console.log(
          yaml.dump({
            ...
            lockedStake: accountInfo.lockedStake ? ethers.utils.formatEther(accountInfo.lockedStake) : '',
            nominatorAddress: accountInfo.nominator,
            currentRewards: accountInfo ? ethers.utils.formatEther(accountInfo.accumulatedRewards.toString()) : '',
            totalPenalty: accountInfo.totalPenalty ? ethers.utils.formatEther(accountInfo.totalPenalty) : '',
            autorestart: nodeConfig.autoRestart,
            stakeState: unstakable,
            stakeable,
    ...
    .action(async (address) => {
      ...
      try {
        ...
        if (eoaData?.operatorAccountInfo?.nominee != null) {
          nodeAccountData = await fetchEOADetails(config, eoaData.operatorAccountInfo.nominee)
        }
  ...
  program
    ...
    .action(async (address) => {
      ...
      try {
        ...
        console.log(error)
        console.error(`Error fetching genesis details for ${address}: ${error}`)
      }
    })
  ...
  setCommand
    ...
    .action((url) => {
      rpcServer.url = url
      // eslint-disable-next-line security/detect-non-literal-fs-filename
      fs.writeFile(path.join(__dirname, `../${File.RPC_SERVER}`), JSON.stringify(rpcServer, undefined, 2), (err) => {
        if (err) console.error(err)
      })
    })
