...
export async function fetchInitialParameters(
  ...
  if (value) {
    ...
    if (parsedValue.nodeRewardAmount && parsedValue.nodeRewardInterval) {
      return {
        nodeRewardAmount: new BN(stripHexPrefix(parsedValue.nodeRewardAmount), 16),
        nodeRewardInterval: new BN(stripHexPrefix(parsedValue.nodeRewardInterval), 16),
      }
    }
  }
  ...
  const nodeRewardAmount = new BN(stripHexPrefix(response.nodeRewardAmountUsd), 16)
  ...
  cache.set('initialParameters', JSON.stringify({ nodeRewardAmount, nodeRewardInterval }), cycleDuration * 1000)
...
export async function getAccountInfoParams(config: networkConfigType, nodePubKey: string) {
  ...
  try {
    ...
    params.lockedStake = nodeData.stakeLock ? new BN(stripHexPrefix(nodeData.stakeLock), 16).toString() : ''
    ...
    params.totalPenalty = new BN(stripHexPrefix(nodeData.nodeAccountStats.totalPenalty), 16).toString()
  } catch (err) {
...
export async function fetchStakeParameters(config: networkConfigType) {
  ...
  const stakeRequired = new BN(stripHexPrefix(stakeParams.stakeRequired.value), 16).toString()
  const cycleDuration = await fetchCycleDuration(config)
