...
describe('tcache', () => {
  ...
  describe('constructor', () => {
    ...
    it('should load data from files if they exist', () => {
      ...
      // Mock file content
      const mockValData = JSON.stringify([['key1', 'value1'], ['key2', 'value2']]);
      const now = new Date();
      const future = new Date(now.getTime() + 10000);
      const mockTimeData = JSON.stringify([['key1', now.toISOString()], ['key2', future.toISOString()]]);
      
      (fs.readFileSync as jest.Mock)
        .mockReturnValueOnce(mockValData)
        .mockReturnValueOnce(mockTimeData);
